...
public abstract class BattleHologramOnlyHealFromSaturationMixin {
    ...
    public float sendShellUpdate(float par1, @Local(argsOnly = true)PlayerEntity player) {
        if (HoloPlayerComponent.KEY.get(player).hologramType.equals(HologramType.BATTLE) || HoloPlayerComponent.KEY.get(player).hologramType.equals(HologramType.BATTLE_DUEL)) {
