...
public abstract class BattleHologramOnlyHealFromSaturationMixin {
    ...
    public float sendShellUpdate(float par1, @Local(argsOnly = true)PlayerEntity player) {
        if (HoloNbtManager.getPlayerState(player).hologramType.equals(HologramType.BATTLE) || HoloNbtManager.getPlayerState(player).hologramType.equals(HologramType.BATTLE_DUEL)) {
