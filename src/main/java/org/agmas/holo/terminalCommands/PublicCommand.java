...
public class PublicCommand extends TerminalCommand{
    @Override
    ...
    public Text run(String cmd, ServerPlayerEntity player) {
        ...
        for (FakestPlayer c : ClonePlayerComponent.KEY.get(player).clones) {
            if (c.holoName.equals(referencedHolo)) {
                HoloPlayerComponent.KEY.get(referencedHolo).publicCamera = true;
                return Text.literal("Made " + c.holoName + "a public camera!").formatted(Formatting.GREEN);
            }
        }
